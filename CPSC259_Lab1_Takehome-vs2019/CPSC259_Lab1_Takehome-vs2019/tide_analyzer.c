...
void run_analysis()
{
  /* Variables */
    ...
    fopen_s(&file_pointer, FILE_NAME, "r");  //open the file and update the position (pointer) ,set the mode to "Read"
    ...
    fft(1,17,readings, complex_component);
        ...
        */
    for (i = 0; i < NUMBER_OF_READINGS; i++) {
        readings[i] = sqrt(pow(readings[i], 2.0) + pow(complex_component[i], 2.0));
    }
        /*
...
void process_file(double array_to_populate[], FILE* pointer_to_data_file)
{
  ...
  // while( fgets (... ) ) {    
    int num1, num2, num3, num4, num5, num6;                                              //initialize variables to store processed values
    while (fgets(line_buffer, LINESIZE, pointer_to_data_file) != NULL) {                  
        i = 0;
        values_per_line = sscanf_s(line_buffer, "%d %d %d %d %d %d", &extracted_values[0], &extracted_values[1], &extracted_values[2], &extracted_values[3], &extracted_values[4], &extracted_values[5]);      
        /*extracted integers from the */
        for (int j = 0; j < values_per_line; j++) {
            array_to_populate[readings_processed] = extracted_values[j];
            readings_processed++;
        }
    }
    printf("%d\n", readings_processed);
