...
void analyze_segments(char* sample_segment, char** candidate_segments, int number_of_candidates, char* output_string)
{
  ...
  for (i = 0; i < number_of_candidates; i++) {
      if (strcmp(*(candidate_segments + i), sample_segment) == 0) {
          has_perfect_match++;
...
int calculate_score(char* sample_segment, char* candidate_segment)
{
  ...
  int trailling = sample_length % 3;
  int sample_length_in_codons = sample_length / 3;
  int i = 0;
  int index = 0;
  char chartempc;
  char chartemps;
  char stringtempc[BUFSIZE];
  char stringtemps[BUFSIZE];
  int codonindexc;
  int codonindexs;

 
  while (index<=(candidate_length-sample_length_in_codons*3)) {  //make sure codon overrun doesn't occur
      temp_score = 0;                                            //reset temp score for each run
      for (i = 0; i < sample_length_in_codons; i++) {
          int temp = 3 * i;
          for (int j = 0; j < 3; j++) {
              *(stringtempc + j) = *(candidate_segment + index + temp + j);        //copy a codon from the candidate
          }
          *(stringtempc + 3) = '\0';                              

          for (int j = 0; j < 3; j++) {
              *(stringtemps + j) = *(sample_segment + temp + j);
          }
          *(stringtemps + 3) = '\0';

          codonindexc = get_codon_index(stringtempc);               //obtain indexes of the codon and use it to find its name
          codonindexs = get_codon_index(stringtemps);

          if (strcmp(stringtemps, stringtempc) == 0)
              temp_score += 10;

          else if (strcmp(*(codon_names + codonindexc), *(codon_names + codonindexs)) == 0)        //see if two codons point to the same acid
              temp_score += 5;

          else{
              for (int j = 0; j < 3; j++) {
                  chartempc = *(stringtempc + j);         
                  chartemps = *(stringtemps + j);
                  if (chartempc == chartemps)
                      temp_score += 2;
                  else if (is_base_pair(chartempc, chartemps) == 1)
                      temp_score += 1;
              }
          }
      }
