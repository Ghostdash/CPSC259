/*
 File:          mazesolver.c
 Purpose:       traverse through a maze and find the most optimal path
 Author:			John Ye
 Student #s:		43883347
 CWLs:      		johnye
 Date:				11/19/2021
...
void process()
{
  ...
  char* current_path = "";                //initialize current_path variable to pass in the the generarte all paths function
  ...
  generate_all_paths(&paths, &paths_found, maze, dimension, 0, 0, current_path);
 ...
 */
int get_maze_dimension( FILE* maze_file )  {                  //works
  ...
  if ( strchr( line_buffer, '\r' ) != NULL ) {
	  dimension = strlen(line_buffer) -2;
	  return dimension;
  } else {
	  dimension = strlen(line_buffer) - 1;
	  return dimension;
  }
}
...
maze_cell** parse_maze( FILE* maze_file, int dimension )
{
  ...
  while ( fgets ( line_buffer, BUFFER, maze_file ) ) {
    for ( column = 0; column < dimension; ++column ) {
		...
		maze[row][column].visited = UNVISITED;                                         //character u mean unvisted
	}
    row++;
  }
	return maze;
...
void generate_all_paths(char*** pathsetref, int* numpathsref, maze_cell** maze, int dimension, int row, int column, char* path)
{
	...
	if (row == dimension || row < 0 || column < 0 || maze[row][column].character == MAZE_WALL
    ...
    /* Otherwise deals with the recursive case.  Pushes the current coordinate onto the path
      ...
      if ( column == ( dimension - 1 ) ) {
	    ...
	    *pathsetref = ( char** ) realloc ( *pathsetref, ( (*numpathsref) + 1 ) * sizeof( char* ) );
	    ...
	    maze[row][column].visited = VISITED;
		generate_all_paths(pathsetref, numpathsref, maze, dimension, row, column + 1, new_path);			       //go right
		generate_all_paths(pathsetref, numpathsref, maze, dimension, row - 1, column, new_path);                   //go up
		generate_all_paths(pathsetref, numpathsref, maze, dimension, row + 1, column, new_path);				   //go down
		generate_all_paths(pathsetref, numpathsref, maze, dimension, row, column - 1, new_path);                   //go left
 ...
 */
int path_cost ( char* path_string )                     //works
{
  int cost = 0;
...
void construct_shortest_path_info ( char** pathset, int numpaths, char* outputbuffer )
{
	...
	for (int i = 0; pathset[0][i] != '\0'; i++) {
		shortest_length++;
	}

	while (count < numpaths) {
		temp_length = 0;
		for (int i = 0; pathset[count][i] != '\0'; i++) {
			temp_length++;
		}
		if (temp_length < shortest_length) {
			index_shortest = count;
			shortest_length = temp_length;
		}
		count++;
	}
	strcpy(temp, "Shortest path: ");
	strcat(temp, pathset[index_shortest]);
...
void construct_cheapest_path_info ( char** pathset, int numpaths, char* outputbuffer )
{
	...
	while (count < numpaths) {
		temp_cost = path_cost(pathset[count]);
		if (temp_cost < cheapest) {
			index_cheapest = count;
			cheapest = temp_cost;
		}
		count++;
	}
	strcpy(temp, "Cheapest path: ");
	strcat(temp, pathset[index_cheapest]);
	sprintf(temp, "%s\nCheapest path cost: %d", temp, path_cost(pathset[index_cheapest]));
	strcpy(outputbuffer,temp);
}
